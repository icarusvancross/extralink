...
const AdBanner300 = memo(({ id }: { id: string }) => (
  <div className="my-8 flex flex-col items-center justify-center w-full">
    ...
    <div className="rounded-[2rem] overflow-hidden shadow-xl border-4 border-white bg-white min-h-[250px] min-w-[300px] flex items-center justify-center">
...
const AdNative = memo(({ id }: { id: string }) => (
  <div className="my-8 w-full max-w-md px-4">
    <p className="text-[7px] text-slate-300 font-black uppercase mb-2 text-center tracking-[0.4em]">Recommended Content</p>
    <div className="rounded-[2rem] overflow-hidden shadow-lg border border-slate-100 bg-white min-h-[350px] flex items-center justify-center">
      <iframe src={`/ad-native.html?v=${id}`} className="w-full h-[400px]" frameBorder="0" scrolling="no"></iframe>
    </div>
  </div>
));
...
export default function WaitingPage() {
  ...
  if (isAdBlockerActive) return (
    <div className="min-h-screen bg-red-600 flex flex-col items-center justify-center p-6 text-center text-white">
      <h1 className="text-4xl font-black mb-4 uppercase">AdBlock Detected!</h1>
      <p className="max-w-md mb-8">Please disable your AdBlocker to continue.</p>
      <button onClick={() => window.location.reload()} className="bg-white text-red-600 px-12 py-4 rounded-full font-black">RELOAD 🔄</button>
    </div>
  );

  if (loading || !linkData) return <div className="min-h-screen bg-slate-950 flex items-center justify-center text-blue-500 font-bold animate-pulse uppercase tracking-widest text-sm text-center">Initializing ExtraLink...</div>;
  ...
  return (
    <div onClick={() => { setHasStarted(true); setIsPaused(false); }} className="min-h-screen bg-slate-50 flex flex-col items-center font-sans cursor-pointer pb-80 relative overflow-x-hidden">
      
      {/* 🚀 إضافة الـ Social Bar هنا ليعمل دائماً */}
      <Script src="https://pl28859100.effectivegatecpm.com/fe/4c/47/fe4c47fb58ff46e2395b8f4a2ec3ceac.js" strategy="lazyOnload" />

      {(!hasStarted || isPaused) && (
        <div className="fixed inset-0 z-[100] bg-slate-900/95 flex items-center justify-center p-6 backdrop-blur-xl text-white">
          <div className="text-center animate-pulse">
            <span className="text-7xl mb-6 block">👆</span>
            <h2 className="text-3xl font-black uppercase">Verify Identity</h2>
            <p className="font-bold">Click anywhere to continue</p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white p-10 rounded-[3rem] shadow-2xl max-w-md w-full text-center border border-slate-100 z-10 mx-4 mt-10 relative overflow-hidden">
        ...
        {count === 0 && (
          <div className="py-6 space-y-4">
            ...
            {showRealButton && <p className="text-red-600 font-black text-xs animate-bounce mt-4 uppercase tracking-widest italic">👇 Scroll to the bottom 👇</p>}
          </div>
        )}
      </div>
      ...
      <div className="max-w-md w-full px-8 mt-10 space-y-24 text-center text-slate-400">
        <div className="space-y-4">
          <h3 className="font-black text-slate-800 uppercase text-xs tracking-widest">Secure Cloud Protection</h3>
          <p className="text-[11px] leading-relaxed font-medium italic opacity-70">"Every link is scanned with AES-256 protocol to ensure end-to-end security."</p>
        </div>

        {/* 3. إعلان Native وسط الصفحة */}
        <AdNative id="mid-native" />

        <div className="grid grid-cols-2 gap-4">
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm"><p className="text-green-500 font-black text-xl italic uppercase leading-none">99.9%</p><p className="text-[7px] font-bold uppercase text-slate-400 mt-2">Uptime</p></div>
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm"><p className="text-blue-500 font-black text-xl italic uppercase leading-none">Verified</p><p className="text-[7px] font-bold uppercase text-slate-400 mt-2">Safe</p></div>
        </div>

        {/* 4. إعلان مربع قبل الزر */}
        <AdBanner300 id="pre-btn" />
      ...
      <footer className="mt-40 opacity-20 grayscale font-black text-[7px] tracking-[0.6em] text-center px-10 pb-10 uppercase">
        ExtraLink Protocol v4.2.1-STABLE
      </footer>
    </div>
  );
}