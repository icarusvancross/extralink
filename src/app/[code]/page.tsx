...
// --- مكونات الإعلانات المعزولة (تضمن الظهور الفوري والثبات) ---
...
export default function WaitingPage() {
  ...
  // 3. مستشعر التركيز لاستئناف العداد فوراً
  useEffect(() => {
  ...
  if (loading || !linkData) return <div className="min-h-screen bg-slate-950 flex items-center justify-center text-blue-500 font-bold animate-pulse uppercase tracking-[0.3em] text-center">INITIALIZING SECURE PROTOCOL...</div>;
  if (isBlocked) return <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-10 text-center font-black text-white uppercase tracking-tighter"><span className="text-6xl mb-6">🛑</span><h1 className="text-3xl text-red-500">Daily Limit Reached</h1></div>;
  ...
  return (
    <div onClick={handleStartInteraction} className="min-h-screen bg-slate-50 flex flex-col items-center font-sans cursor-pointer pb-96 relative overflow-x-hidden">
      ...
      {(!hasStarted || isPaused) && (
        <div className="fixed inset-0 z-[100] bg-slate-900/95 flex items-center justify-center p-6 backdrop-blur-xl text-white text-center">
          <div className="animate-pulse">
            ...
            <p className="font-bold text-sm text-slate-400 italic">Click anywhere to continue</p>
          </div>
        </div>
      )}

      <header className="w-full bg-white p-6 text-center border-b border-slate-200 sticky top-0 z-40 shadow-sm font-black text-blue-600 italic text-2xl tracking-tighter">ExtraLink</header>
      ...
      <div className="bg-white p-10 rounded-[3rem] shadow-2xl max-w-md w-full text-center border border-slate-100 z-10 mx-4 mt-4 relative overflow-hidden">
        ...
        <div className="py-10 text-[7rem] font-black text-slate-800 tabular-nums leading-none tracking-tighter">{count}</div>
        
        {count === 0 && (
          <div className="py-6 space-y-4">
            <button onClick={(e) => { e.stopPropagation(); setShowRealButton(true); }} className="w-full bg-blue-600 text-white font-black py-7 rounded-[2rem] text-2xl uppercase shadow-xl transition-all hover:scale-105">CONTINUE</button>
            {showRealButton && <p className="text-red-600 font-black text-xs animate-bounce mt-4 uppercase tracking-widest italic">👇 Scroll to the bottom to find Next Page 👇</p>}
          </div>
        )}
      </div>

      {/* 2. إعلان مربع تحت العداد - يظهر فوراً */}
      <AdBanner300 id="mid-banner-1" />
      ...
      <div className="max-w-md w-full px-8 mt-10 space-y-24 text-center text-slate-400">
        <div className="space-y-4">
          ...
          <p className="text-[11px] leading-relaxed font-medium opacity-70 italic">Verifying connection... ExtraLink uses global AES-256 bit encryption protocol for your safety.</p>
        </div>

        {/* 3. إعلان Native منتصف الصفحة - يظهر فوراً */}
        <AdNative id="mid-native-2" />

        <div className="grid grid-cols-2 gap-4">
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex flex-col items-center">
            <p className="text-green-500 font-black text-xl italic leading-none uppercase">99.9%</p>
            <p className="text-[7px] font-bold uppercase text-slate-400 mt-2">Uptime</p>
          </div>
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex flex-col items-center">
            <p className="text-blue-500 font-black text-xl italic leading-none uppercase">Verified</p>
            <p className="text-[7px] font-bold uppercase text-slate-400 mt-2">Safe</p>
          </div>
        </div>

        {/* 4. إعلان مربع فوق الزر - يظهر فوراً */}
        <AdBanner300 id="mid-banner-3" />
        ...
        {/* 5. إعلان Native القاع - يظهر فوراً */}
        <AdNative id="bottom-native-3" />
      </div>
